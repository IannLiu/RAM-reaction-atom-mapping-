...
def match_Hs(r_info:list,
    ...
    if sum(Hs_number) !=0:
        ...
        Sep_Hs = True # If not balanced H for heavy atoms, H migrated from/to heavy atoms
        warnings.warn("Some hydrogen atoms might migrate from/to [H] or [H][H], however, these atoms might be ambiguous")
    ...
    r_nums = re.findall(r"\d+", r_smiles)
    replace_nums = [[str(i+1), num] for i, num in enumerate(r_nums)]
    r_smiles_copy = copy.deepcopy(r_smiles)
    for replace_num in replace_nums:
        r_smiles_copy = r_smiles_copy.replace(":"+replace_num[1] + ']', ":r" + replace_num[0] + ']')
    r_smiles_copy = r_smiles_copy.replace(":r", ":")
    p_smiles_copy = copy.deepcopy(p_smiles)
    for replace_num in replace_nums:
        p_smiles_copy = p_smiles_copy.replace(":"+replace_num[1] + ']', ":r" + replace_num[0] + ']')
    p_smiles_copy = p_smiles_copy.replace(":r", ":")
    
    return str_to_mol(r_smiles_copy), str_to_mol(p_smiles_copy)
